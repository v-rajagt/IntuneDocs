...
If you want to evaluate a cloud-managed modern desktop in your own organization, this [guided scenario](~/fundamentals/guided-scenarios-overview.md) predefines all the necessary configurations for a basic deployment. In this guided scenario, you will create a secure environment where you can try out Intune device management capabilities. 
...
- (Optional) [Register the test device with Windows Autopilot](enrollment-autopilot.md#add-devices).
...
Windows Autopilot automates the configuration of new devices so that users can set up them up out of box, without IT assistance. If your device is already registered with Windows Autopilot, select it by its serial number. For more information about using Windows Autopilot, see [Register device with Windows Auto pilot (Optional)](guided-scenarios-cloud-managed-pc.md#register-device-with-windows-auto-pilot-optional).
...
The user and device will be added to new security groups. They'll also be configured with Intune-recommended settings for security and productivity at work or school. After the user joins the device to Azure AD, additional apps and settings will be added to the device. To learn more about these additional configurations, see [Quickstart: Enroll your Windows 10 device](quickstart-enroll-windows-device.md).
...
1. Verify that the selected is assigned MDM user scope
    - Ensure [MDM User scope](windows-enroll.md#enable-windows-10-automatic-enrollment) is:
...
- If you are using Windows Defender Advanced Threat Detection, create an [Intune Compliance policy](advanced-threat-protection.md#create-and-assign-the-compliance-policy) to require Defender threat analysis to meet compliance.
- Create a [Device-based Conditional Access policy](advanced-threat-protection.md#create-a-conditional-access-policy) to block access if the device does not meet Intune compliance.
